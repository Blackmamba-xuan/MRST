...
import datetime
import math
from SAC.env_laneChange import Env
import time
from tensorboardX import SummaryWriter
from SAC.sac_torch import Agent
from SAC.env_laneKeep import Env

if __name__ == '__main__':
    log_dir = 'checkpoints_SAC_11_14_keeplane/'
    run_dir = log_dir + 'logs/'
    logger = SummaryWriter(str(log_dir))
    env=Env()
    input_dim = (4, 120, 200)
    action_dim = 2
    agent = Agent(input_dims=input_dim, env=env,max_action=[0.08,0.1],
    ...
    for ep_i in range(n_episodes):
        ...
        for ep_t in range(episode_length):
            ...
            next_state, next_speed ,reward, done,successFlag = env.step(action, time_step=ep_t)
            ...
            if done:
                break
        print('successful flag: ', successFlag)
