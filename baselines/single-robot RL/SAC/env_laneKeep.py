...
class Env(object):
    def __init__(self):
        ...
        self.init_speed=[0.06,0.2]
        self.init_pos_list=[[-0.805519,0.284848],[-0.796213,0.177235],[-0.783987,-0.189060],[-0.769798,-0.328783],
                            [-0.577271,0.518433],[-0.576790,0.246540],[-0.550677,-0.067753],[-0.549804,-0.356617]]
        ...
        self.im_frames=[]
        self.stop_counter=0
        self.max_step_per_round = 5

    def reset(self):
        print('enter reset')
        self.target_speed = [0.06, 0]
        self.speed_record = []
    ...
    def step(self, action,time_step,laneKeep='Slow'):
        print('the action is: ', action)
        if laneKeep=='Slow':
            self.target_speed[0]=np.clip(action[0], 0.04, 0.08)
        else:
            self.target_speed[0] = np.clip(action[0], 0.08, 0.14)
        self.target_speed[1]=np.clip(action[1], -0.1, 0.1)
        twist=Twist()
        twist.linear.x=self.target_speed[0]
        twist.linear.y =0
        twist.linear.z=0
        ...
        time.sleep(0.6)
        curr_pos=self.pos
        dist=curr_pos[0]-self.init_pos[0]
        print('dist: ', dist)
        successFlag=0
        if abs(dist)>0.003:
            done=True
            reward=-20
        elif time_step==(self.max_step_per_round-1):
            successFlag=1
            done=True
            reward=20
        else:
