...
if __name__ == '__main__':
    log_dir = 'checkpoints_DQN_7_15_minSpeed_0.04_env300_env650/'
    ...
    for i_episode in range(num_episodes):
        ...
        for et_i in range(epi_length):
            ...
            speed.to("cuda:0")
            reward_sum += reward
            reward = torch.tensor([reward], device="cuda:0").float()
            if done:
                next_state = None
                next_speed = None
            else:
        ...
        if i_episode % agent.TARGET_UPDATE == 0:
            ...
            agent.target_net.load_state_dict(agent.policy_net1.state_dict())
