...
class Agent(object):
    ...
    def __init__(self, h, w, n_actions):
        ...
        self.steps_done = 0
        self.policy_net1 = DQN(h, w, n_actions).to(device)
        self.policy_net2 = DQN(h, w, n_actions).to(device)
        self.target_net = DQN(h, w, n_actions).to(device)
        self.n_actions=n_actions

        self.optimizer1 = optim.RMSprop(self.policy_net1.parameters())
    ...
    def select_action(self,state, speed):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                return (self.policy_net1.forward(state,speed)+self.policy_net2.forward(state,speed)).max(1)[1].view(1, 1)
        else:
    ...
    def learn(self,logger=None):
        ...
        state_action_values = 1/2*(self.policy_net1(state_batch,speed_batch)+self.policy_net2(state_batch,speed_batch)).gather(1, action_batch)
        ...
        loss.backward()
        for param in self.policy_net1.parameters():
