...
class Scenario(object):
    ...
    def reset(self):
        ...
        try:
            ...
            self.eneities[0].pos=[robot6_initPos[0],robot6_initPos[1]]
            ...
            self.eneities[1].pos = [robot7_initPos[0], robot7_initPos[1]]
            ...
            self.eneities[2].pos = [robot8_initPos[0], robot8_initPos[1]]

        except:
            print("set model state Service call failed: %s")
        time.sleep(0.2)
        socialAgentList=[0,2]
        self.socialAgentID = socialAgentList[np.random.randint(2)]
        ...
        for i, action in enumerate(actions):
            if i == self.socialAgentID:
                ...
                agent_dists=[math.sqrt((self.eneities[j].pos[0] - social_agent.pos[0]) ** 2 +
                                      (self.eneities[j].pos[1] - social_agent.pos[1]) ** 2) for j in range(self.nagents) if j!=i]
...
class Entity(object):
    ...
    def __init__(self,name,pos,lane_flag):
        ...
        self.pub_target_vel = rospy.Publisher(name + '/setTargetVel', Float64, queue_size=1)
