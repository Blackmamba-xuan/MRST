...
class ComaAgent(object):
    """
    General class for COMA agents (policy, critic, target policy, target
    critic, exploration noise)
    """
    ...
    def step(self, obs, ac, explore=False, hidden_state=None):
        """
        Take a step forward in environment for a minibatch of observations
        Inputs:
            obs (PyTorch Variable): Observations for this agent
        ...
        """
        h_out=None
        pol_in=torch.cat((obs,ac), dim=1)
        if self.use_rnn:
            action, h_out = self.policy.forward(pol_in,hidden_state)
        else:
            action = self.policy.forward(pol_in)
        explore=False
        if self.discrete_action:
            if explore:
                action = gumbel_softmax(action, hard=True)
...
class A2CAgent(object):
    ...
    def __init__(self, num_in_pol, num_out_pol, num_in_critic, num_out_critic=1, hidden_dim=64,
        ...
        if use_rnn:
            ...
            self.policy = DiscretePolicy(num_in_pol, num_out_pol,hidden_dim=hidden_dim)
